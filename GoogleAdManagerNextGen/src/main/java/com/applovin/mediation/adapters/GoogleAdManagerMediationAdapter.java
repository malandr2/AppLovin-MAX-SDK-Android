...
public class GoogleAdManagerMediationAdapter
        ...
        implements MaxSignalProvider, MaxInterstitialAdapter, MaxAppOpenAdapter, MaxRewardedAdapter, MaxAdViewAdapter,
        MaxNativeAdAdapter {
    ...
    public GoogleAdManagerMediationAdapter(final AppLovinSdk sdk) {
        super(sdk);
    }
    ...
    public void initialize(final MaxAdapterInitializationParameters parameters, @Nullable final Activity activity,
        ...
        if (initialized.compareAndSet(false, true)) {
            ...
            InitializationConfig config = new InitializationConfig.Builder(applicationId)
                    ...
                    // NOTE: If MAX makes an ad request to AdMob, and the AdMob account has AL
                    // enabled (e.g. AppLovin Bidding) _and_ detects the AdMob<->AppLovin adapter,
                    // AdMob will still attempt to initialize AppLovin
            ...
            new Thread(() -> {
                if (parameters.getServerParameters().getBoolean("init_without_callback", false)) {
                    ...
                    MobileAds.initialize(context, config, initializationStatus -> {
                        ...
                        final AdapterStatus.InitializationState googleAdsState = googleAdsStatus != null
                                ? googleAdsStatus.getInitializationState()
                                : null;
    ...
    private SignalRequest createSignalRequest(MaxAdapterSignalCollectionParameters parameters, Context context) {
        ...
        if (adFormat == MaxAdFormat.BANNER || adFormat == MaxAdFormat.LEADER || adFormat == MaxAdFormat.MREC) {
            ...
            boolean isAdaptive = isAdaptiveBannerObj instanceof String
                    && "true".equalsIgnoreCase((String) isAdaptiveBannerObj);
    ...
    public void showInterstitialAd(final MaxAdapterResponseParameters parameters, @Nullable final Activity activity,
        ...
        if (interstitialAd != null) {
            if (activity == null) {
                ...
                listener.onInterstitialAdDisplayFailed(new MaxAdapterError(MaxAdapterError.AD_DISPLAY_FAILED, 0,
                        "Activity is required to show GMA Next-Gen ads"));
                return;
            }
    ...
    @Override
    public void loadAppOpenAd(final MaxAdapterResponseParameters parameters, @Nullable final Activity activity,
            final MaxAppOpenAdapterListener listener) {
        String placementId = parameters.getThirdPartyAdPlacementId();
        String bidResponse = parameters.getBidResponse();
        boolean isBiddingAd = AppLovinSdkUtils.isValidString(bidResponse);
    ...
    @Override
    public void showAppOpenAd(@NonNull final MaxAdapterResponseParameters parameters, @Nullable final Activity activity,
            @NonNull final MaxAppOpenAdapterListener listener) {
        String placementId = parameters.getThirdPartyAdPlacementId();
        ...
        if (appOpenAd != null) {
            if (activity == null) {
                ...
                listener.onAppOpenAdDisplayFailed(new MaxAdapterError(MaxAdapterError.AD_DISPLAY_FAILED, 0,
                        "Activity is required to show GMA Next-Gen ads"));
                return;
            }
    ...
    public void showRewardedAd(final MaxAdapterResponseParameters parameters, @Nullable final Activity activity,
        ...
        if (rewardedAd != null) {
            if (activity == null) {
                ...
                listener.onRewardedAdDisplayFailed(new MaxAdapterError(MaxAdapterError.AD_DISPLAY_FAILED, 0,
                        "Activity is required to show GMA Next-Gen ads"));
                return;
            }
    ...
    public void loadAdViewAd(final MaxAdapterResponseParameters parameters, final MaxAdFormat adFormat,
        ...
        log("Loading " + (isBiddingAd ? "bidding " : "") + (isNative ? "native " : "") + adFormat.getLabel()
                + " ad for placement id: " + placementId + "...");

        Context context = getContext(activity);

        if (isNative) {
            NativeAdViewListener nativeAdViewListener = new NativeAdViewListener(parameters, adFormat, activity,
                    listener);
            if (isBiddingAd) {
                NativeAdLoader.loadFromAdResponse(bidResponse, nativeAdViewListener);
            } else {
                NativeAdRequest.Builder requestBuilder = new NativeAdRequest.Builder(placementId,
                        Arrays.asList(NativeAd.NativeAdType.NATIVE))
                        .setAdChoicesPlacement(getAdChoicesPlacement(parameters));
                NativeAdRequest nativeAdRequest = applyCommonRequestParameters(requestBuilder, parameters, context)
                        .build();
            ...
            AdViewListener adViewListener = new AdViewListener(placementId, adFormat, adView, activity, adSize,
                    listener);
    ...
    private boolean isInlineAdaptiveBanner(final MaxAdapterParameters parameters) {
        ...
        return (adaptiveBannerType instanceof String)
                && ADAPTIVE_BANNER_TYPE_INLINE.equalsIgnoreCase((String) adaptiveBannerType);
    }
    ...
    private AdFormat toAdFormat(final MaxAdapterSignalCollectionParameters parameters) {
        ...
        // NOTE: App open ads were added in AppLovin v11.5.0 and must be checked after
        // all the other ad formats to avoid throwing an exception
        else if (adFormat == MaxAdFormat.APP_OPEN) {
    ...
    private <T extends BaseRequestBuilder<T>> T applyCommonRequestParameters(T builder, MaxAdapterParameters parameters,
            Context context) {
        Bundle serverParameters = parameters.getServerParameters();
        ...
        // "applovin_dv360" is for DV360_BIDDING, which is a separate bidder from
        // regular ADMOB_BIDDING
        ...
        Object neighbouringContentUrlStringsObject = localExtraParameters
                .get("google_neighbouring_content_url_strings");
        ...
        if (customTargetingDataObject instanceof Map) {
            // try-catching unsafe cast to List<String> or Map<String, Object> in case an
            // incorrect type is set.
            try {
    ...
    /**
     * Update the global mute state for AdMob - must be done _before_ ad load to
     * restrict inventory which requires playing with volume.
     */
    ...
    private AdChoicesPlacement getAdChoicesPlacement(MaxAdapterResponseParameters parameters) {
        // Publishers can set via nativeAdLoader.setLocalExtraParameter(
        // "admob_ad_choices_placement", ADCHOICES_BOTTOM_LEFT );
        final Map<String, Object> localExtraParams = parameters.getLocalExtraParameters();
        final Object adChoicesPlacementObj = localExtraParams != null
                ? localExtraParams.get("admob_ad_choices_placement")
                : null;
    ...
    private class AdViewListener
        ...
        AdViewListener(final String placementId, final MaxAdFormat adFormat, final AdView adView,
                @Nullable final Activity activity, final AdSize adSize, final MaxAdViewAdapterListener listener) {
            this.placementId = placementId;
    ...
    private class NativeAdViewListener
        ...
        NativeAdViewListener(final MaxAdapterResponseParameters parameters, final MaxAdFormat adFormat,
                @Nullable final Activity activity, final MaxAdViewAdapterListener listener) {
        ...
        public void onAdFailedToLoad(@NonNull final LoadAdError loadAdError) {
            MaxAdapterError adapterError = toMaxError(loadAdError);
            log("Native " + adFormat.getLabel() + " ad (" + placementId + ") failed to load with error: "
                    + adapterError);
    ...
    private class NativeAdListener
        ...
        public NativeAdListener(final MaxAdapterResponseParameters parameters, final Context context,
                final MaxNativeAdAdapterListener listener) {
                ...
                public void run() {
                    ...
                    if (mediaContent != null) {
                        ...
                        if (mediaImageDrawable != null) {
                            ...
                            mediaContentAspectRatio = (float) mediaImageDrawable.getIntrinsicWidth()
                                    / (float) mediaImageDrawable.getIntrinsicHeight();
                        }
                    }
    ...
    private class MaxGoogleNativeAd
        ...
        public boolean prepareForInteraction(final List<View> clickableViews, final ViewGroup container) {
            ...
            // Plugins
                ...
                //
                // Logic required for proper media view rendering in plugins (e.g. Flutter /
                // React Native)
                //
                ...
                // NOTE: Will be false for React Native (will extend `ReactViewGroup`), but true
                // for Flutter
                boolean hasPluginLayout = (pluginContainer instanceof RelativeLayout
                        || pluginContainer instanceof FrameLayout);
                ...
                if (mediaView != null) {
                    ...
                    if (mediaView instanceof MediaView) {
                        ...
                        if (!hasPluginLayout) {
                            ...
                            if (googleMediaContent != null && googleMediaContent.getHasVideoContent()) {
                                AutoMeasuringFrameLayout wrapper = new AutoMeasuringFrameLayout(
                                        pluginContainer.getContext());
                                wrapper.addView(googleMediaView, new ViewGroup.LayoutParams(
                    ...
                    ViewGroup.LayoutParams mediaViewLayout = new ViewGroup.LayoutParams(
                            ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
                ...
                if (hasPluginLayout) {
                    ViewGroup.LayoutParams nativeAdViewLayout = new ViewGroup.LayoutParams(
                            ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
    ...
    private static class AutoMeasuringFrameLayout
            extends FrameLayout {
        AutoMeasuringFrameLayout(final Context context) {
            super(context);
        }
